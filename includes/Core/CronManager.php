...
class CronManager {
	...
	public function __construct() {
		add_action( 'init', array( $this, 'registerFiltersAndActions' ) );
	...
	public function handleCronExecution() {
		if ( wp_doing_cron() ) {
			add_action( $this->import_hook, array( $this, 'processImportTasks' ) );
			add_action( $this->export_hook, array( $this, 'processExportTasks' ) );
			add_action( $this->sync_hook, array( $this, 'processSyncTasks' ) );
			add_action( $this->import_custom_hook, array( $this, 'processUserCustomImport' ) );
			add_action( $this->export_custom_hook, array( $this, 'processUserCustomExport' ) );
		}
	}

	public function processUserCustomImport() {
		global $wpdb;
		$meli_listing_ids_arr = array();
		...
		try {
			...
			foreach ( $items_to_process as $item ) {
				...
				try {
					...
					$meli_listing_ids_arr = array_merge( $meli_listing_ids_arr, array( $item->meli_listing_id ) );

					UserListingToImport::update_user_listing_item_import_status( array( $item->meli_listing_id ), 'finished' );
			...
			foreach ( $items_to_process as $item ) {
				...
				try {
					...
					if ( isset( $item->meli_user_id ) && ! empty( $item->meli_user_id ) ) {
						...
						$custom_template_data = Template::selectCustomTemplateData( $item->template_id, array( 'seller_meli_id' ) );
	...
	private function get_pending_items_to_process( $taskType ) {
		...
		$current_items_to_process = array();

		$current_process = Process::getCurrentProcessData( $taskType, array( 'processing', 'pending' ) );

		if ( ! $current_process ) {
			if ( $taskType == 'import' || $taskType == 'export' || $taskType == 'sync' ) {

				// TODO if is import export o sync get total items to process
				$current_process_id = Process::createProcess( $taskType, array() );
			} else {

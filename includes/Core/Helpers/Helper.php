...
class Helper {
	...
	public static function change_meli_listing_status( $product_id, $status ) {
		...
		$response = $meli->put( "/items/{$asoc_listing}", array( 'status' => $status ), array( 'access_token' => $seller_data->access_token ) );
	...
	public static function load_partial( $template_path, $data = array(), $once = true, $print = true ) {
	...
	public static function handleLoadMeliCategories() {
		...
		if ( empty( $seller_id ) ) {
			wp_send_json_error( array( 'message' => 'Seller ID is required' ) );
			wp_die();
		}
		...
		if ( ! $seller_data ) {
			wp_send_json_error( array( 'message' => 'Seller data not found' ) );
			wp_die();
		}
		...
		if ( empty( $category_id ) ) {
			$response   = $meli->get( '/sites/' . $seller_data->site_id . '/categories', array( 'access_token' => $seller_data->access_token ) );
			$categories = $response['body'];
		} else {
			$response = $meli->get( '/categories/' . $category_id, array( 'access_token' => $seller_data->access_token ) );
		...
		$options = self::load_partial( 'includes/Core/Views/Partials/meliconnect_product_edit_category_options.php', array( 'categories' => $categories ), true, false );

		// Cargar la vista y obtener el HTML generado
		$path_from_route_html = self::load_partial( 'includes/Core/Views/Partials/meliconnect_product_edit_category_path.php', $data, true, false );
	...
	public static function handleUpdateMeliCategory() {
		...
		if ( empty( $category_id ) || empty( $woo_product_id ) ) {
			wp_send_json_error( array( 'message' => 'Category ID or product ID is required' ) );
			wp_die();
		}
		...
		if ( empty( $template_id ) ) {
			...
			if ( ! $template_id ) {
				wp_send_json_error( array( 'message' => 'Error creating template' ) );
				wp_die();
			}
		...
		if ( $template_data ) {
			Template::updateTemplateData( $template_id, array( 'category_id' => $category_id ) );

...
class ProductEdit {
	...
	public $template_attibutes           = array();
	public $meli_category_data           = array();
	public $meli_category_attrs          = array();
	public $meli_category_variable_attrs = array();
	public $meli_category_sale_terms     = array(); // warranty, manufacturing time ,etx
	public $matched_attrs                = array();

	public $select_options = array();
	public $form_values    = array();

	public $listing_types_by_category_and_seller = array();
	public $official_stores_by_seller            = array();
	public $category_shipping_preferences        = array();

	public static $meliConnection = null;
	public static $seller_data    = null;

	public $shipping_modes_names = array();


	public function __construct() {

		add_action( 'add_meta_boxes', array( $this, 'add_meliconnect_meta_box_data' ) );
		add_action( 'add_meta_boxes', array( $this, 'add_meliconnect_meta_box_actions' ) );
		add_action( 'woocommerce_product_options_pricing', array( $this, 'meliconnect_add_general_tab_html' ) );
		add_action( 'woocommerce_product_options_inventory_product_data', array( $this, 'meliconnect_add_stock_tab_html' ) );
		add_filter( 'woocommerce_product_data_tabs', array( $this, 'add_mercadolibre_product_tab' ) );
		add_action( 'woocommerce_product_data_panels', array( $this, 'add_mercadolibre_product_tab_content' ) );
		add_action( 'woocommerce_product_data_panels', array( $this, 'add_meliconnect_logs_product_tab_content' ) );

		add_action( 'woocommerce_product_options_attributes', array( $this, 'add_mercadolibre_attrs_content' ) );

		add_action( 'woocommerce_after_product_attribute_settings', array( $this, 'match_mercadolibre_atts' ), 10, 2 );

		add_action( 'woocommerce_process_product_meta', array( $this, 'meliconnect_save_custom_product_data' ), 10, 1 );

		add_action( 'woocommerce_admin_process_variation_object', array( $this, 'meliconnect_woocommerce_save_product_variation' ), 10, 2 );

		add_action( 'woocommerce_variation_options', array( $this, 'meliconnect_custom_variation_fields' ), 10, 3 );

		$this->shipping_modes_names = array(
		...
		$this->select_options = array(
			'buying_modes'             => array( 'buy_it_now' => esc_html__( 'Buy It Now', 'meliconnect' ) ),
			'listing_types'            => array(
			...
			'channels'                 => array( 'mercadolibre' => esc_html__( 'Mercadolibre', 'meliconnect' ) ),
			'shipping_methods'         => array(
				'me2'           => esc_html__( 'Mercado Envíos - Standard ', 'meliconnect' ) . '(' . esc_html__( 'ME2', 'meliconnect' ) . ')',
				'me1'           => esc_html__( 'Mercado Envíos - Express ', 'meliconnect' ) . '(' . esc_html__( 'ME1', 'meliconnect' ) . ')',
				'not_specified' => esc_html__( 'No Shipping Configuration', 'meliconnect' ),
			),
			'manufacturing_time_units' => array(
				'días' => esc_html__( 'días', 'meliconnect' ),
			),
			'status'                   => array(
	...
	public function meliconnect_woocommerce_save_product_variation( $variation, $i ) {
		if ( ! isset( $_POST['meliconnect_variation_nonce'] ) || ! check_admin_referer( 'meliconnect_save_product_variation_nonce', 'meliconnect_variation_nonce' ) ) {
		...
		$template_id  = get_post_meta( $parent_id, 'meliconnect_asoc_template_id', true );
		...
		foreach ( $template_variations as $variation_key => $variation_values ) {
			...
			if ( isset( $variation_values['variation_data']['disable_sync'] ) && $variation_values['variation_data']['disable_sync'] == 1 ) {
				update_post_meta( $variation_id, 'meliconnect_meli_asoc_variation_sync_disabled', 1 );
			} else {
				update_post_meta( $variation_id, 'meliconnect_meli_asoc_variation_sync_disabled', 0 );
			}
	...
	public function meliconnect_custom_variation_fields( $loop, $variation_data, $variation ) {
		$this->set_product_vars();

		$current_variation_attrs = $this->filterVariationAttributes( $variation_data );

		echo '<div class="meliconnect_meli_attribute_info_container">';
		...
		wp_nonce_field( 'meliconnect_save_product_variation_nonce', 'meliconnect_variation_nonce' );
		...
		foreach ( $current_variation_attrs as $name => $attr_value ) {
			...
			$meli_attributes_normalized = array_map(
				function ( $key ) {
			...
			if ( $meli_attr_index !== false ) {
				...
				echo "<div class='meliconnect_variation_row'>";
		...
		$variation_sync_is_disabled = get_post_meta( $variation->ID, 'meliconnect_meli_asoc_variation_sync_disabled', true );
		...
		echo '<input type="checkbox" name="template[variations][' . esc_attr( $loop ) . '][variation_data][disable_sync]" class="meliconnect_variation_disable_sync" value="1" ' . esc_attr( $checked_disabled ) . '>';
	...
	public function mapVariationWithMeliVariations( $variation, $current_variation_attrs ) {
		...
		$meli_variation_id = get_post_meta( $variation->ID, 'meliconnect_meli_asoc_variation_id', true );
	...
	public function meliconnect_save_attributes( $post_id ) {
	...
	private static function getSellerData() {
		...
		if ( $post && $post->post_type === 'product' ) {
			...
			$seller_id      = get_post_meta( $woo_product_id, 'meliconnect_meli_seller_id', true );
			if ( ! empty( $seller_id ) ) {
	...
	public function set_product_vars() {
		...
		$template_id = get_post_meta( $this->woo_product_id, 'meliconnect_asoc_template_id', true );
		...
		$meli_category_id = get_post_meta( $this->woo_product_id, 'meliconnect_meli_category_id', true );
		...
		if ( ! empty( $meli_category_id ) ) {
			...
			if ( ! empty( $meli_category_attrs ) ) {
				...
				$this->meli_category_variable_attrs = $this->getAttributesWithTag( $meli_category_attrs, array( 'allow_variations' ) );
			}
			...
			if ( ! empty( $meli ) && ! empty( $seller_data ) ) {
				...
				if ( isset( $meli_sale_terms_response['body'] ) && is_array( $meli_sale_terms_response['body'] ) ) {
					...
					$meli_category_sale_terms = array();
				}
			}
	...
	public function getAttributesWithTag( $meli_category_attrs, $tags_filter = array( 'allow_variations' ) ) {
		$meli_category_variable_attrs = array();
	...
	public function formatCategoryData( $meli_category_data ) {
		...
		if ( isset( $meli_category_data['settings']['buying_modes'] ) ) {
			$this->select_options['buying_modes'] = array();
		...
		if ( isset( $meli_category_data['settings']['currencies'] ) ) {
			$this->select_options['currencies'] = array();
	...
	public function formatChannels( $seller_data ) {
		if ( isset( $seller_data->has_mercadoshops ) && $seller_data->has_mercadoshops ) {
			$this->select_options['channels'] = array(
		...
		if ( ! empty( $sale_terms ) && is_iterable( $sale_terms ) ) {
			foreach ( $sale_terms as $term ) {
				...
				if ( $term->id == 'MANUFACTURING_TIME' ) {
					$manufacturing_allowed_units = array();
	...
	public function formatCategoryShippingPreferencesResponse( $category_shipping_preferences ) {
		...
		$formatted_category_shipping_preferences = array();
	...
	public function formatOfficialStroresResponse( $official_stores_by_seller ) {
		$formatted_official_stores_by_seller = array();
		$oficial_sotres_select               = array();

		foreach ( $official_stores_by_seller as $official_store ) {
			$formatted_official_stores_by_seller[ $official_store->official_store_id ] = array(
	...
	public function formatListingTypesResponse( $listing_types_by_category_and_seller ) {
		...
		$formatted_listing_types_by_category_and_seller = array();
	...
	public function getPendingRequiredAttrs() {
		...
		$pending_required_attrs_names = array();
	...
	public function isRequiredAttribute( $meli_attr ) {
		...
		return isset( $meli_attr['tags']['required'] )
			...
			|| ( isset( $meli_attr['id'] ) && in_array( $meli_attr['id'], array( 'BRAND', 'MODEL' ) ) );
	}
	...
	public function add_mercadolibre_product_tab_content() {
		...
		$template             = isset( $this->template_data ) ? $this->template_data : array();
	...
	public function add_meliconnect_logs_product_tab_content() {
		$item_export_error = get_post_meta( $this->woo_product_id, 'meliconnect_export_meli_errors', true );

		$item_export_error = ! empty( $item_export_error ) ? maybe_unserialize( $item_export_error ) : array();

		$last_export_time = get_post_meta( $this->woo_product_id, 'meliconnect_last_export_time', true );
		$error_time       = get_post_meta( $this->woo_product_id, 'meliconnect_export_meli_error_time', true );
		...
		$logs_view_data = array(
			...
			'last_json_sent'           => get_post_meta( $this->woo_product_id, 'meliconnect_last_export_json_sent', true ),
		);
	...
	public function add_meliconnect_meta_box_data() {
		...
		add_meta_box(
			...
			array( $this, 'display_meliconnect_meta_box_data' ), // Función para mostrar el contenido
			'product',
	...
	public function add_meliconnect_meta_box_actions() {
		...
		$meli_listing_id = get_post_meta( $post->ID, 'meliconnect_meli_listing_id', true );
		...
		add_meta_box(
			...
			array( $this, 'display_meliconnect_meta_box_actions' ), // Función para mostrar el contenido
			'product',
			'side', // Contexto, puedes usar 'normal', 'side' o 'advanced'
			'high' // Prioridad
		);
	...
	public function display_meliconnect_meta_box_data( $post ) {
		// Get postmeta values
		$meli_listing_id = get_post_meta( $post->ID, 'meliconnect_meli_listing_id', true );
		$meli_permalink  = get_post_meta( $post->ID, 'meliconnect_meli_permalink', true );
		$template_id     = get_post_meta( $post->ID, 'meliconnect_asoc_template_id', true );
		$seller_id       = get_post_meta( $post->ID, 'meliconnect_meli_seller_id', true );
		$meli_status     = get_post_meta( $post->ID, 'meliconnect_meli_status', true );
	...
	public static function handleImportSingleListing() {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'meliconnect_import_single_listing_nonce' ) ) {
	...
	public static function handleExportSingleListing() {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'meliconnect_export_single_listing_nonce' ) ) {
			wp_send_json_error( esc_html__( 'Invalid nonce', 'meliconnect' ) );
			return;
		}
	...
	public static function handleUnlinkSingleListing() {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'meliconnect_unlink_single_listing_nonce' ) ) {
			wp_send_json_error( esc_html__( 'Invalid nonce', 'meliconnect' ) );
			wp_die();
		}
		...
		if ( empty( $new_status ) || $update_status ) {
			...
			wp_send_json_error( array( 'message' => esc_html__( 'Failed to update product status', 'meliconnect' ) ) );
		}

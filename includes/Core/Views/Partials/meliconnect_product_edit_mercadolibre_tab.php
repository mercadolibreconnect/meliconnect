<div id="mercadolibre_product_data" class="melicon-product-edit-meli-tab wc-metaboxes-wrapper panel woocommerce_options_panel hidden">
    <div class="options_group">
        <p class="form-field melicon_general_seller_meli_id_field">
            ...
            echo '<pre>' . var_export($select_options, true) . '</pre>';
            echo PHP_EOL . '-------------------  FINISHED  ---------------------' . PHP_EOL;

            echo PHP_EOL . '-------------------- $form_values --------------------' . PHP_EOL;
            echo '<pre>' . var_export($form_values, true) . '</pre>';
            echo PHP_EOL . '-------------------  FINISHED  ---------------------' . PHP_EOL;
    ...
    <div id="melicon-meessage-category-container" class="options_group melicon_show_if_change_category melicon-m-1">
        <?php wc_print_notice(__('You must select a category and save product template to see category options', 'meliconnect'), 'notice');  ?>
    </div>
    <div class="options_group melicon_hide_if_change_category ">
        <p class="form-field melicon_general_buying_mode_field">
            ...
            woocommerce_wp_select(array(
                ...
                'label' => __('Buying Mode', 'meliconnect'),
        ...
        <p class="form-field melicon_general_listing_type_id_field">
            ...
            woocommerce_wp_select(array(
                ...
                'label' => __('Listing Type', 'meliconnect'),
        ...
        <p class="form-field melicon_general_condition_field">
            ...
            woocommerce_wp_select(array(
                ...
                'label' => __('Condition', 'meliconnect'),
        ...
        <p class="form-field melicon_general_warranty_type_field">
            <label for="template[meta][warranty_type]"><?php _e('Warranty', 'meliconnect'); ?> </label>
            <span class="wrap">
                <select id="template[warranty_type]" name="template[meta][warranty_type]">
                    <?php foreach ($select_options['warranty_types'] as $key => $value) { ?>
                        <option value="<?php echo $key; ?>" <?php selected($key, $form_values['warranty_type']); ?>> <?php echo $value; ?></option>
                    <?php } ?>
                </select>
                <input id="melicon_general_warranty_time" style="max-width:60px" class="input-text wc_input_decimal" type="number" min="0" name="template[meta][warranty_time]" value="<?php echo esc_attr($form_values['warranty_time']); ?>" />
                <select id="template[warranty_time_unit]" name="template[meta][warranty_time_unit]">
                    <?php foreach ($select_options['warranty_time_units'] as $key => $value) { ?>
                        <option value="<?php echo $key; ?>" <?php selected($key, $form_values['warranty_time_unit']); ?>> <?php echo $value; ?></option>
                    <?php } ?>
                </select>
            </span>
        </p>
        ...
        <p class="form-field melicon_general_shipping_method_field">
            ...
            woocommerce_wp_select(array(
                ...
                'label' => __('Shipping Method', 'meliconnect'),
        ...
        <p class="form-field template[meta][local_pick_up]_field ">
            <label for="template[meta][local_pick_up]"><?php _e('Local Pick Up', 'meliconnect'); ?></label>
            <input type="checkbox" name="template[meta][local_pick_up]" id="template[meta][local_pick_up]" value="1" class="checkbox"
                <?php echo (isset($form_values['local_pick_up']) && $form_values['local_pick_up'] == '1') ? 'checked' : ''; ?>>
            <span class="description"><?php _e('Enable Local Pick Up', 'meliconnect'); ?></span>
        </p>
        <p class="form-field template[meta][free_shipping]_field ">
            <label for="template[meta][free_shipping]"></label>
            <input type="checkbox" name="template[meta][free_shipping]" id="template[meta][free_shipping]" value="1" class="checkbox"
            <?php echo (isset($form_values['free_shipping']) && $form_values['free_shipping'] == '1') ? 'checked' : ''; ?>>
            <span class="description"><?php _e('Enable Free Shipping', 'meliconnect'); ?></span>
        </p>
        ...
        <p class="form-field melicon_general_manufacturing_time_field">
            <label for="template[manufacturing_time]"><?php _e('Manufacturing Time', 'meliconnect'); ?> </label>
            <span class="wrap">
                <input id="melicon_general_manufacturing_time" style="max-width:60px" class="input-text wc_input_decimal" type="number" min="0" max="45" name="template[meta][manufacturing_time]" value="<?php echo esc_attr($form_values['manufacturing_time']); ?>" />
                <select id="template[manufacturing_time_unit]" name="template[meta][manufacturing_time_unit]">
                    <?php foreach ($select_options['manufacturing_time_units'] as $key => $value) { ?>
                        <option value="<?php echo $key; ?>" <?php selected($key, $form_values['manufacturing_time_unit']); ?>> <?php echo $value; ?></option>
                    <?php } ?>
                </select>
            </span>
        </p>
        ...
        <?php if (!empty($select_options['official_stores'])) { ?>
            <p class="form-field melicon_general_official_store_field">
                ...
                woocommerce_wp_select(array(
                    ...
                    'label' => __('Official Store', 'meliconnect'),
        ...
        <p class="form-field template[meta][catalog_listing]_field">
            <label for="template[meta][catalog_listing]"><?php _e('Catalog Listing', 'meliconnect'); ?></label>
            <input type="checkbox" name="template[meta][catalog_listing]" id="template[meta][catalog_listing]" value="1" class="checkbox"
                <?php echo (isset($form_values['catalog_listing']) && $form_values['catalog_listing'] == '1') ? 'checked' : ''; ?>>
            <span class="description"><?php _e('Enable Catalog Listing', 'meliconnect') ?></span>
        </p>
        ...
        <p class="form-field melicon_general_status_field">
            ...
            woocommerce_wp_select(array(
                ...
                'label' => __('Listing Status', 'meliconnect'),
    ...
    <div class="options_group ">
        <p class="form-field melicon_general_title_structure_field">
            <label for="melicon_general_title_structure">
                <?php _e('Title Structure', 'meliconnect'); ?>
            </label>
        ...
        <p class="form-field melicon_general_description_structure_field">
            <label for="melicon_general_description_structure">
                <?php _e('Description Structure', 'meliconnect'); ?>
            </label>
        ...
        <p class="form-field melicon_general_how_price_formed">
            ...
            woocommerce_wp_select(array(
                ...
                'label' => __('How is the price formed? (*)', 'meliconnect'),
                'options' => array(
                    'sale_price' => __('Sale price', 'meliconnect'),
                    'regular_price' => __('Only regular price', 'meliconnect'),
                ),
        ...
        <p class="form-field melicon_general_has_sync_field">
            ...
            woocommerce_wp_select(array(
                ...
                'label' => __('Has Sync', 'meliconnect'),
                'options' => array(
                    '1' => __('Yes', 'meliconnect'),
                    '0' => __('No', 'meliconnect'),
                ),
    ...
    <div class="options_group melicon_product_edit_mercadolibre_tab_save">
        <div class="toolbar toolbar-buttons">
            <button id="melicon_save_template_button"
                type="button"
                class="button button-primary melicon-button">
                <?php _e('Save Template', 'meliconnect'); ?>
            </button>
        </div>
    </div>
</div>
...
<script>
    var current_category_id = '<?php echo $form_values['category_id']; ?>';
    var woo_product_id = '<?php echo $woo_product_id; ?>';
</script>
...
class Template
{
    ...
    private static $template_listing_metas = [
        'site_id',
        'permalink',
        'user_product_id',
        'family_name',
    ...
    private static $template_custom_metas = [
    ...
    public static $template_woo_post_metas = [
        'currency_id',
        'manufacturing_time',
        'condition',
        'listing_type_id',
    ...
    public static function createProductTemplateAttrsFromPostData($template_post_data, $template_id, $product_id)
    {
        ...
        if (empty($template_post_data['attrs'])) {
            error_log('attrs not found in post data');
            return false;
        }
        ...
        foreach ($template_post_data['attrs'] as $key => $attr) {
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                echo '<pre>' . var_export($data_to_insert, true) . '</pre>';
                echo PHP_EOL . '-------------------  FINISHED  ---------------------' . PHP_EOL;

                echo PHP_EOL . '-------------------- $decoded_attr --------------------' . PHP_EOL;
                echo '<pre>' . var_export($decoded_attr, true) . '</pre>';
                echo PHP_EOL . '-------------------  FINISHED  ---------------------' . PHP_EOL;
                wp_die(); */
                ...
                error_log("Error decodificando el atributo en línea $key: " . json_last_error());
            }
        }
    }
    ...
    {
        $form_checkboxes_keys = ['local_pick_up', 'free_shipping', 'catalog_listing'];
    ...
    public static function addCurrentTemplateMetasToRequest($template_id, &$post)
    {
        ...
        echo '<pre>' . var_export( $added_keys, true) . '</pre>';
        echo PHP_EOL . '-------------------  FINISHED  ---------------------' . PHP_EOL;
    ...
    public static function createUpdateTemplateFromMeliListing($used_by, $used_asoc_id, $meli_listing_data, $template_parent_id = null)
    {
        ...
        $channels = isset($meli_listing_data->channels) ? $meli_listing_data->channels : [];
        ...
        $data = [
            ...
            'short_description'  => __('Template autogenerated from meli listing', 'meliconnect'),
    ...
    public static function createUpdateTemplate($data)
    {
        if (!isset($data['used_by']) || empty($data['used_by'])) {
            error_log("Error creating template: used_by is required");
            return false;
        }

        if (!isset($data['used_asoc_id']) || empty($data['used_asoc_id'])) {
            error_log("Error creating template: used_asoc_id is required");
            return false;
        }
        ...
        if ($existing_template) {
            ...
            if ($wpdb->last_error) {
                error_log("Error updating template: ");
            ...
            if ($wpdb->last_error) {
                error_log("Error creating template: ");
                error_log($wpdb->last_error);
                error_log("SQL: " . $wpdb->last_query);
                return false;
            }
    ...
    public static function deleteCreateTemplatesMetasFromMeliListing($template_id,  $meli_listing_data, $template_data = [])
    {
        self::deleteTemplatesMetas($template_id);
    ...
    public static function deleteTemplatesMetas($template_id, $meta_keys = ['all'])
    {
        ...
        if (in_array('all', $meta_keys)) {
            ...
            if ($wpdb->last_error) {
                error_log("Error deleting all template metas: " . $wpdb->last_error);
                return false;
            }
        ...
        $sql = $wpdb->prepare(
            ...
            array_merge([$template_id], $meta_keys)
        );

        $deleted = $wpdb->query($sql);

        if ($wpdb->last_error) {
            error_log("Error deleting specific template metas: ");
            error_log($wpdb->last_error);
            error_log("SQL: " . $wpdb->last_query);
            return false;
        }
    ...
    public static function createTemplateMetasFromMeliListing($template_id, $meli_listing_data)
    {
        ...
        $sale_terms = isset($meli_listing_data->sale_terms) ? $meli_listing_data->sale_terms : [];
    ...
    public static function createTemplateMeta($template_id, $meta_key, $meta_value)
    {
        ...
        $data = ['meta_key' => $meta_key, 'meta_value' => $meta_value, 'template_id' => $template_id];

        $wpdb->insert($template_metas_table_name, $data);

        if ($wpdb->last_error) {
            error_log("Error creating template meta: " . $wpdb->last_error);
    ...
    public static function deleteCategoryRelatedDataFromTemplate($template_id)
    {
        ...
        if ($wpdb->last_error) {
            error_log("Error deleting template category: ");
            error_log($wpdb->last_error);
            error_log("SQL: " . $wpdb->last_query);
            return false;
        }
        ...
        self::deleteTemplatesMetas($template_id, ['currency_id', 'listing_type_id', 'shipping', 'condition', 'catalog_domain', 'catalog_listing', 'official_store_id', 'warranty', 'permalink']);
    ...
    public static function deleteTemplate($template_id)
    {
        ...
        if ($wpdb->last_error) {
            error_log("Error deleting template: ");
            error_log($wpdb->last_error);
            return false;
        }
    ...
    public static function deleteTemplatesAttributes($template_id)
    {
        ...
        if ($wpdb->last_error) {
            error_log("Error deleting template attrs: ");
            error_log($wpdb->last_error);
            error_log("SQL: " . $wpdb->last_query);
            return false;
        }
    ...
    public static function createUpdateTemplateAttributes($used_by, $used_asoc_id, $meli_attribute_id, $data)
    {
        ...
        if ($existing_row) {
            ...
            if ($result === false) {
                ...
                error_log('Error updating the record in the table ' . $template_attributes_table_name);
            ...
            if ($result === false) {
                // En caso de error, registrar el error en el log y la última consulta SQL
                error_log('Error inserting a new record in the table ' . $template_attributes_table_name);
                error_log('Last SQL: ' . $wpdb->last_query);
                error_log('Data sent: ' . print_r($data, true));
            }
        }
    ...
    public static function createMainProductAttributes($template_id, $meli_listing_data, $woo_product_id)
    {
        ...
        if (!$product) {
            error_log("Product with ID $woo_product_id not found.");
            return false;
        }
        //error_log('main attrs');
        ...
        foreach ($meli_listing_data->variations as $variation) {
            ...
            if (isset($variation->attribute_combinations) && !empty($variation->attribute_combinations)) {
                foreach ($variation->attribute_combinations as $attr_combination) {
                    ...
                    if ($wpdb->last_error) {
                        error_log("Error creating variation combination product templates attributes: ");
                        error_log($wpdb->last_error);
                        error_log("SQL: " . $wpdb->last_query);
                        return false;
                    }
    ...
    public static function insert_template_attributes($data, $table_name)
    {
        ...
        if ($wpdb->last_error) {
            error_log("Error creating template attributes: ");
            error_log($wpdb->last_error);
            error_log("SQL: " . $wpdb->last_query);
            return false;
        }
    ...
    public static function get_woo_attribute_id($product, $attribute_name)
    {
        ...
        //error_log("Product attributes: " . print_r($product_attributes, true));

        foreach ($product_attributes as $attr) {
            //error_log("-------------------");
            //error_log("Attribute name received: " . wc_sanitize_taxonomy_name($attribute_name));
            //error_log("Attribute name: " . $attr->get_name());

            if (strcasecmp($attr->get_name(), 'pa_' . wc_sanitize_taxonomy_name($attribute_name)) === 0 || strcasecmp($attr->get_name(), $attribute_name) === 0) {
                //error_log("Attribute found: " . $attr->get_id());
                return $attr->get_id();
            }
        }
    ...
    public static function selectCustomTemplateData($template_id, array $data)
    {
        ...
        if (is_string($data)) {
            $data = [$data]; // Convertir string a array
        }
    ...
    public static function updateTemplateData($template_id, array $data)
    {
        ...
        $updated = $wpdb->update(
            ...
            ['%d']              // Formato del where (en este caso, %d porque es un id numérico)
        );

        if ($wpdb->last_error) {
            error_log("Error updating template data: ");
            error_log($wpdb->last_error);
            error_log("SQL: " . $wpdb->last_query);
            return false;
        }

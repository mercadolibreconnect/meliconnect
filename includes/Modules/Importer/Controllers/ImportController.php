...
class ImportController implements ControllerInterface {
	...
	public static function initImportProcess( $meli_listings_ids = array() ) {
	...
	public static function multiGetListingsData( $seller_data, $meli_user_listings_ids ) {
		...
		$meli_user_listings_with_data = array();
		$listings_with_errors         = array();
	...
	public static function getMeliUserLisntingIds( $seller_data ) {
		...
		for ( $i = 1; $i <= $total_pages; $i++ ) {
			...
			} elseif ( isset( $current_items['body']->error ) && ( $current_items['body']->error == 'client.exception' ) ) {
					break;
			}
		}
		...
		foreach ( $product_ids as $product_id ) {
			...
			if ( $product && $product->is_type( 'variable' ) ) {
				...
				foreach ( $variations as $variation_id ) {
					...
					if ( $variation ) {
						$variable_products[ $product_id ] = array(
